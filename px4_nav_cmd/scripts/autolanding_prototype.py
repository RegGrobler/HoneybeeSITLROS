            ...
            for cnt in contours:
                ...
                if len(approx) <= 5 and len(approx) > 3 and area >= area_min and area <= area_max:
                    ...
                    if pxl_m_ratio == 0:							
                        ...
                        f = open("/home/honeybee/TrackMarkerTest.txt","a+") 
                        f.write("%f,%f,%f,%f,%f,%f,%f,\n" % (self.circle_marker_sp.position.x,self.circle_marker_sp.position.y,self.marker_sp.position.z,self.local_pos.z,current_time,self.local_pos.x,self.local_pos.y))
                        f.close()
    ...
    while mission_flag and not rospy.is_shutdown():
        ...
        if state_machine_flag == 1:
            if  cnt.circle_marker_spotted == 1:
                ...
                if abs(cnt.local_vel.x) < threshold and abs(cnt.local_vel.y) < threshold and abs(cnt.local_vel.z) < threshold and abs(cnt.circle_marker_sp.position.x) < threshold and abs(cnt.circle_marker_sp.position.y) < threshold and abs(cnt.local_pos.z) > cnt.commanded_altitude-0.1:
                    ...
                    print("State 2")
            elif cnt.circle_marker_spotted == 0:
                print("Marker Lost\n\n")
        ...
        if state_machine_flag == 2:
            if time.time() - last_time > 5:
                cnt.updateSp(cnt.local_pos.y,cnt.local_pos.x,-6,yaw) #Here the UAV is commanded to fly altitude of 6m
                publish_setpoint(cnt,sp_pos_pub) #publish yaw of 0
                rate.sleep()
            else:    
                ...
                publish_marker_setpoint(cnt,marker_rel_pub) # publish marker positions
                rate.sleep()
                cnt.updateSp(cnt.local_pos.y,cnt.local_pos.x,-3.5,yaw) #Here the UAV is commanded to fly altitude of 6m
                publish_setpoint(cnt,sp_pos_pub) #publish yaw of 0
                rate.sleep()
                if  abs(cnt.marker_sp.position.x) < threshold and abs(cnt.marker_sp.position.y) < threshold  and abs(cnt.local_pos.z)<3.6:
