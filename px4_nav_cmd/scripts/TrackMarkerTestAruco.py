...
class Controller:
    ...
    def arucoCb(self,msg):
	    ...
	    f = open("/home/honeybee/ArucoHoverTest.txt","a+") 
            f.write("%f,%f,%f,%f,%f,%f,%f,\n" % (self.marker_sp.position.x,self.marker_sp.position.y,self.marker_sp.position.z,self.local_pos.z,current_time,self.local_pos.x,self.local_pos.y))
            f.close()
        except:
            #self.marker_id = 0
            pass
    ...
    def imageCb(self,msg):
	    ...
	    area_min = -self.a_min_480*np.exp(-self.b_min_480*abs(height))+self.c_min_480 - 400 	#calculate the minimum area +300 was used for simulated test
			...
			self.circle_marker_sp.position.z = self.local_pos.z						#Z_position of marker is set to the current height of the uav using estimated values
                    													#This should make no difference as it was disabled in the PX4 software				
                    self.circle_marker_spotted = 1									#Marker has been spotted
                    self.last_time = time.time()									#last time marker was spotted is updated

                    cv2.drawContours(cv_image, [approx], 0, (50,255,0), 2)						#Draws the marker //Can be removed
                    
                elif time.time() - self.last_time >= 5:  #Has the marker stopped being identified for more than 5 seconds 
                    self.circle_marker_spotted = 0
		elif abs(self.local_pos.z) >= 10:        #If the UAV flies too high for some reason then enter bailout state
		    self.circle_marker_spotted = -1
...
def run(argv):
    ...
    while not (cnt.state.mode == "OFFBOARD" or rospy.is_shutdown()):
        ...
        k=0
        while k<10:
 	    cnt.updateSp(cnt.local_pos.y,cnt.local_pos.x,-cnt.local_pos.z,yaw) #Here the UAV is commanded to fly altitude of 8m
    ...
    while mission_flag and not rospy.is_shutdown():
		...
		#print(cnt.marker_id)
		last_time = time.time() 
                cnt.updateMarkerSp(cnt.marker_sp.position.x,cnt.marker_sp.position.y,-cnt.marker_sp.position.z,1) # Here the position of the marker is being published to the FCU. Marker Spotted is set to 1
                publish_marker_setpoint(cnt,marker_rel_pub) # publish marker positions
		rate.sleep()
                cnt.updateSp(cnt.local_pos.y,cnt.local_pos.x,-cnt.commanded_altitude,yaw) #Here the UAV is commanded to fly altitude of 6m
		...
		rate.sleep()
                cnt.updateSp(cnt.local_pos.y,cnt.local_pos.x,-5,yaw) #Let the drone hover on the spot untill the marker is seen
                publish_setpoint(cnt,sp_pos_pub)
                rate.sleep()
	    ...
	    elif cnt.circle_marker_spotted == -1:
		print("UAV flew too high\n\n")
                cnt.updateMarkerSp(0,0,0,0) #switch back to normal control
                publish_marker_setpoint(cnt,marker_rel_pub)
		rate.sleep()
