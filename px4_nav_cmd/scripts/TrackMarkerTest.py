...
class Controller:
    ...
    def imageCb(self,msg):
        try:
            ...
            _, threshold = cv2.threshold(gray, 240, 255, cv2.THRESH_BINARY)	#threshold image to remove black and white
            _, contours, _ = cv2.findContours(threshold, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)	#contour the image
	    area_min = -self.a_min_480*np.exp(-self.b_min_480*abs(self.local_pos.z))+self.c_min_480 + 300 	#calculate the minimum area
            area_max = -self.a_max_480*np.exp(-self.b_max_480*abs(self.local_pos.z))+self.c_max_480 + 1000 	#calculate the maximum area
            #contour_count = 1
			...
			#self.circle_marker_sp.position.z = self.local_pos.z						#Z_position of marker is set to the current height of the uav using estimated values
                    	self.circle_marker_sp.position.z = 6 								#This should make no difference as it was disabled in the PX4 software
...
def run(argv):
    ...
    while mission_flag and not rospy.is_shutdown():
		...
		print("UAV flew too high\n\n")
                ...
                publish_marker_setpoint(cnt,marker_rel_pub)
		rate.sleep()
		cnt.updateSp(0,0,-4,yaw) #Let the drone hover on the spot untill the marker is seen
                publish_setpoint(cnt,sp_pos_pub)
                rate.sleep()
