...
threshold = 0.2 # How small the position error and velocity should be before sending the next waypoint
...
x = 3 #the x position of the gps location sent tp the drone
z = 7   #the z position of the aruco marker
yaw = 0 #commanded yaw
timeout_time = 4
marker_hover_z = 0.8
...
class Controller:
    ...
    def circleMarkerCb(self,msg):
        try:
            #print(self.circle_marker_spotted)
            if msg.transforms[0].child_frame_id == "ellipses_detection/t0-0":
                if self.circle_marker_error_flag == 1:
                    if (msg.transforms[0].transform.translation.x - self.circle_marker_sp.position.x >= error) or (msg.transforms[0].transform.translation.y - self.circle_marker_sp.position.y >= error):
                        pass
                    else:
                        self.circle_marker_sp.position.x = msg.transforms[0].transform.translation.x
                        self.circle_marker_sp.position.y = -msg.transforms[0].transform.translation.y
                        self.circle_marker_sp.position.z = msg.transforms[0].transform.translation.z
                else:
                        self.circle_marker_sp.position.x = msg.transforms[0].transform.translation.x
                        self.circle_marker_sp.position.y = -msg.transforms[0].transform.translation.y
                        self.circle_marker_sp.position.z = msg.transforms[0].transform.translation.z
                ...
                self.counter = 0
            elif msg.transforms[0].child_frame_id == "fcu_frd":
...
def run(argv):
    ...
    rospy.Subscriber('tf',TFMessage,cnt.circleMarkerCb)
    ...
    while mission_flag and not rospy.is_shutdown():
        ...
        #print("circle marker spotted: %d" % cnt.circle_marker_spotted)
        #print(cnt.marker_sp.position.x)
        f = open("followcircle.txt","a+") 
        ...
        if state_machine_flag == 1:
            ...
            rate.sleep()
            #if cnt.marker_id == 1:
            if  cnt.circle_marker_spotted == 1: #is the marker actually seen for now we just assume that it is.
                ...
                cnt.updateSp(cnt.local_pos.y,cnt.local_pos.x,-7,yaw) # here the only value being published to the drone is a yaw of 0. The other values are the current values of the drone.
                ...
                if abs(cnt.local_vel.x) < threshold and abs(cnt.local_vel.y) < threshold and abs(cnt.local_vel.z) < threshold and abs(cnt.circle_marker_sp.position.x) < threshold and abs(cnt.circle_marker_sp.position.y) < threshold:
                    ...
                    cnt.circle_marker_error_flag = 1 #now errors will be ignored
                    print("3 - The drone is stable above the circle marker\n")
                    print("3 - The altitude will now be lowered\n")
            else:
        ...
        if state_machine_flag == 2: # in this state the marker has been lost and the drone is comanded back to its gps location without the use of marker data
            ...
            if abs(cnt.local_pos.x - x) < threshold + 0.5 and abs(cnt.local_vel.x) < threshold +0.5 and abs(cnt.local_pos.y - y) < threshold +0.5 and abs(cnt.local_vel.y) < threshold + 0.5 and abs(cnt.local_pos.z - z) < threshold + 0.5 and abs(cnt.local_vel.z) < threshold + 0.5 :
                ...
                cnt.land_flag = 1
                print("1 - Lets Retry \n\n")
                 #now the Callback function will look for errors.
        
        if state_machine_flag == 3: #In this state the drone tracks the circle marker but lowers altitude so that the aruso marker can come into view once the height threshold has been reached then go to state machine 4
            cnt.updateSp(cnt.local_pos.y,cnt.local_pos.x,-cnt.local_pos.z,yaw) #Let the drone hover on the spot untill the marker is seen
            publish_setpoint(cnt,sp_pos_pub)
            rate.sleep()
            #if cnt.marker_id == 1:
            if cnt.circle_marker_spotted == 1:
                ...
                if  abs(cnt.local_vel.z) < vision_threshold and abs(cnt.circle_marker_sp.position.z-marker_hover_z) < threshold:
                    ...
                    print("4 - The drone will now switch over to the aruco marker\n")
            else:
                if time.time()-last_time >= timeout_time:
                    state_machine_flag = 2
                else:
        ...
        if state_machine_flag == 4: #in this state the drone is tasked to localize with the aruco marker
            if cnt.marker_id == 1:
                ...
                rate.sleep()
                last_time = time.time()
                if abs(cnt.local_vel.x) < vision_threshold and abs(cnt.local_vel.y) < vision_threshold and abs(cnt.local_vel.z) < vision_threshold and abs(cnt.marker_sp.position.x) < vision_threshold and abs(cnt.marker_sp.position.y) < vision_threshold:
        ...
        if state_machine_flag == 5:
            if cnt.marker_id == 1:
                cnt.updateMarkerSp(cnt.marker_sp.position.x,cnt.marker_sp.position.y,cnt.marker_sp.position.z,1)
                publish_marker_setpoint(cnt,marker_rel_pub)
                cnt.updateSp(cnt.local_pos.y,cnt.local_pos.x,-0,yaw) 
                publish_setpoint(cnt,sp_pos_pub) 
                rate.sleep()
                last_time = time.time()
            else:
        ...
        if state_machine_flag == 6:
            if cnt.marker_id == 1:
                cnt.updateMarkerSp(0,0,0,0)
                publish_marker_setpoint(cnt,marker_rel_pub)
                cnt.updateSp(cnt.local_pos.y,cnt.local_pos.x,-cnt.local_pos.z,yaw) 
                publish_setpoint(cnt,sp_pos_pub)
                rate.sleep()
                last_time = time.time()
